...
To jest cały UI wyjątków. Backend dopytuje maksymalnie jednym krokiem, nie drzewem 20 pól.
...
Completion siedzi kontekstowo na karcie tasku; etykieta wynika z typu tasku.
...
1. Wszystkie eventy tabeli 1 wolno zapisać offline — **wszystkie**. Dyrektywy STOP po odzyskaniu sieci renderują się przed INFO.
...
4. Worker działa według **ostatniej dyrektywy, którą zACK-ował**, z zastrzeżeniem safety: wyjątek nie przywraca niebezpiecznego starego kroku. Dyrektywa tylko `CHANNEL_ACCEPTED` nie zmienia jego obowiązującego planu.
5. Evidence (głos, zdjęcie) jest niemutowalne. Agent może dodać interpretację obok, nie zamiast.
6. `SAFE_HOLD / BLOCKED_PENDING_RESOLUTION` nie znika przez restart, retry/sync ani rollover dnia.

---

## Dozwolone reakcje agenta — krótki kaganiec

Bez człowieka agent **może**:
...
- uwolnić workera przy `TECHNICAL_WAIT`;
- wykonać end-of-day **wyłącznie** z drzewa polityki i dopiero po jego spełnieniu przejść do `DAY_CLOSED`;
...
- zdjąć `SAFE_HOLD` przez restart/nowy dzień/sync;
...
A–L są zamknięte. Wracamy do nich tylko wtedy, gdy implementacja lub deterministyczny test kontraktu ujawni prawdziwą sprzeczność.

Pełna kolejka Given/When/Then, w tym poprawione S7 oraz trwałość SAFE_HOLD w S31, znajduje się w [`M2_CONTRACT_TESTS_QUEUE.md`](./M2_CONTRACT_TESTS_QUEUE.md).

Formalny zapis decyzji znajduje się w [`M2_FREEZE_RECORD.md`](./M2_FREEZE_RECORD.md).

Kolejny suchy krok: testy kontraktu -> zielony critical suite -> event models / JSON Schema / TypeSpec -> backend contract -> cienkie MobileWC UI -> powrót produktowy do M3.
